...
- The app's own tests now run in a real browser too (`flutter test --platform chrome`, nightly on `main` and as a gate on the release tag rather than on every pull request), but compiled with DDC, not `dart2js` — the compiler the deployed web build actually uses. DDC and `dart2js` disagree on some integer-width edges, so a `dart2js`-only runtime bug in app code could still reach production undetected by this run. The pure packages run under real `dart2js` in a browser, and the web build is compiled on every change, so a compile-time web break cannot merge. Both bugs of that kind so far — a 64-bit hash, and a random draw with a bit shift — would now be caught, but only because the arithmetic in question lives in the pure packages, which is a rule (Recorded in [`docs/adr/0009-web-platform-coverage.md`](docs/adr/0009-web-platform-coverage.md)) rather than a guarantee. The browser run also uses an in-memory WASM database rather than the OPFS-and-worker path a real deploy opens.
