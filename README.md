...
**The contrast warning informs and does not block.** The colour picker shows a WCAG ratio and says plainly when it is too low to read comfortably, then lets the reader proceed. Someone with light sensitivity may want low contrast deliberately, and overriding that in an app whose whole premise is configurability would be worse than a warning they can ignore. The point is that nobody arrives at dark grey on black without being told. The fade control works the same way: a crossfade set longer than a word is held would draw two words on top of each other at the anchor, and the setting says so rather than refusing the value.

**The settings preview is the reading surface, not a picture of it.** The preview draws through the same widget the reader reads with, running a real playback session over one sample sentence. Two renderers for one thing had already come apart: the surface painted one set of polarity colours while the contrast readout measured another, so the WCAG figure in settings described a colour pair the app never put on screen. Sharing the constants would have fixed the numbers and left the cause.

**Front matter is skipped, never removed.** Dropping blocks would shift every block id and invalidate saved positions, and a wrong guess would delete real text with no way back. Detection only reports a suggested opening index, so the reader can rewind into the licence page if they want it. When that index came from pattern matching rather than a marker the book itself supplied, the book says so on opening and offers the start of the file — a guess the reader is never shown is one they have no way to correct, which is the same reason an unresolvable saved position is reported rather than silently restarted.
...
- The settings preview runs over one fixed sample sentence rather than the reader's own book, so length-scaled pacing previews against English word lengths whatever the reader actually reads.
- The contrast readout judges the text against the background and does not measure the fixation letter, which carries its own fixed colour and could sit close to a tinted background.
...
- On web that re-parse blocks the interface. `compute()` does not spawn an isolate on that target — it runs the function on the main thread and wraps it in a Future — so importing or opening a book freezes the page for as long as the parse takes. On Android and Windows the same call genuinely offloads.
...
- Front matter detection uses an explicit marker where a book provides one, and pattern matching otherwise. The pattern path is a guess, capped at fifteen percent of a book and never destructive, and the reader is told when it was used and offered the start of the file. A book carrying a *wrong* explicit marker gets no such notice, because the app has no reason to doubt it.
