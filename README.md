...
- [x] Reading positions written while reading, not only on close: on every stop, every fifteen seconds the reader is moving, when the app is hidden, and when the book closes. Recorded in [`docs/adr/0011-position-saving.md`](docs/adr/0011-position-saving.md)
...
- [x] Chrome that follows the reading profile: the controls, chapter panel and progress bar drawn over the text take the polarity the reader chose, rather than a single light theme sitting on a dark surface
- [x] App appearance: light, dark or whatever the device asks for, six accents, and a high contrast mode that replaces the neutral ramp rather than deepening it. Stored on the device and read before the first frame, so a dark-theme device does not flash white on the way in. Recorded in [`docs/adr/0012-app-chrome-appearance.md`](docs/adr/0012-app-chrome-appearance.md)
...
**A position is written while reading, not only when the book closes.** A place recorded once per sitting is lost to a crash, a killed app or a closed browser tab. It is now written on every deliberate stop, every fifteen seconds while the index is moving, when the app is hidden, and on close. The outbox coalesces unsent position events per book, so the extra cadence costs nothing on the wire: an intermediate position has no consumer, since the service resolves to the latest and other devices want where the reader ended up. Recorded in [`docs/adr/0011-position-saving.md`](docs/adr/0011-position-saving.md).
...
**The chrome follows the reading profile, not the platform.** The controls, chapter button, chapter panel, progress bar and front matter notice are all drawn directly on top of the text. Under a single app theme they were light, so the two central field loss presets — both reverse polarity — put a light panel and light buttons into the visual field of the reader most likely to have chosen reverse polarity to keep bright light out of it. Chrome brightness is read from the reading surface's measured luminance rather than from the polarity setting: polarity decides the ink, and the background is separately tintable to anything, but chrome sits on the surface and only has to stay legible against what is behind it. The library, settings and sign-in follow the reader's own appearance settings instead, since none of them sits on a reading surface. Those settings never reach the reading surface either: the reader chrome theme seeds from a fixed neutral of its own rather than from the app accent, so a reader who tinted their background moss and set the accent to rust does not find the two adjacent on the one screen whose colours they chose.
...
**The reading surface is a button, and the word is not spoken word by word.** The surface is the app's primary control — play, pause, and advance under elicited pacing — so it announces as a button with a label that follows playback state. The current word is offered only while the stream is stopped. A reader using RSVP is reading with their eyes, and speech arriving four times a second would fight the visual stream rather than describe it; anyone who needs speech instead of sight is better served by a whole book read aloud than by one word at a time. A word reached by pausing, advancing or rewinding is different: one fact, and one the reader just asked for.
...
**Word timing comes from a timer, not a frame callback.** `PlaybackSession` chains `Timer` directly rather than driving from a `Ticker`. That was chosen so `fakeAsync` could step a whole paragraph in microseconds, but it also means how long a word is held does not quantise to whatever frame rate the platform happens to be delivering — which turned out to matter on the web, where the browser decides how many frames a page gets.
...
**A failed start is shown, not left blank.** Everything the app needs before its first write — the database, the stored session, the clock — is awaited before the first frame, because every write needs a stamp and a half-started app would save positions that cannot be ordered. That means a failure there has no widget tree to report itself through, and on the web an uncaught one draws a blank page indistinguishable from a hang. The chain runs inside a catch that closes any half-open database and renders a screen naming the error, with a retry.
