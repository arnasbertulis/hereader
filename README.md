...
Every substantial decision has an ADR carrying the alternatives that were rejected and why; the [index below](#architecture-decision-records) lists all seventeen. These are the ones that shape the most code.
...
- Jumping to a chapter and then pressing play steps back by the profile's `rewindWords`, landing a word or two into the previous chapter. The tap zones and the sentence and paragraph jumps do not have this fault — they go through `stopAt`, which resumes from where the reader chose — but chapter navigation still uses `seekToIndex`, which keeps playing when it was playing and is what that screen wants. Fixing it means changing two behaviours at once. See ADR 0020.
...
- The reader chrome, the library controls and the note editor have been verified on Windows only. Nothing in any of them animates or translates a large area, which is the shape the Chrome throttle above actually reaches, but that is an expectation rather than a result.
