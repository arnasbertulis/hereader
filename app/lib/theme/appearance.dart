...
abstract final class AppearanceKeys {
  ...
  // Kept as 'chrome_text_scale' for backward compatibility with stored settings.
  static const textSize = 'ui.chrome_text_scale';
}
...
const double textSizeMin = 0.85;
const double textSizeMax = 1.5;
...
class AppearanceSettings {
  ...
  @override
  int get hashCode => Object.hash(themeMode, accent, highContrast, textSize);
}
...
/// `NumberFormat`.
String encodeTextSize(double value) => value.toStringAsFixed(2);
...
double decodeTextSize(String? value) {
  final parsed = value == null ? null : double.tryParse(value);
  if (parsed == null || parsed < textSizeMin || parsed > textSizeMax) {
    return AppearanceSettings.defaults.textSize;
  }
  return parsed;
}
...
class AppearanceController extends ChangeNotifier {
  ...
  Future<void> restore() async {
    ...
    final textSize = await repository.preference(AppearanceKeys.textSize);
    ...
    _settings = AppearanceSettings(
      ...
      textSize: decodeTextSize(textSize),
    );
  ...
  Future<void> setTextSize(double value) async {
    if (value == _settings.textSize) return;

    await _persist(AppearanceKeys.textSize, encodeTextSize(value));
